...
**Forbidden patterns** (enforced by [`test/parity/batch-request-manager-ordering.test.ts`](test/parity/batch-request-manager-ordering.test.ts) and related parity tests): `readEpochInvestorDecimals`, `requireVaultInvestorDecimals`, `if (investorDecimals)`, `if (!decimals) return`, `decimals missing` warn strings.
