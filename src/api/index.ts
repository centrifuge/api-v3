...
import schema, { Blockchain, CrosschainPayload, Token, TokenInstance } from "ponder:schema";
...
app.get("/routes", sValidator("query", routesParams), async (c) => {
  ...
  const blockchainByCentId = new Map(blockchains.map((b) => [b.centrifugeId, b]));
  ...
  const routes = nonHubTokenInstanceRows.flatMap((row) => {
    ...
    const spokeBlockchain = blockchainByCentId.get(row.token_instance.centrifugeId);
    ...
    return [
      ...
      ...(nonHubTokenInstanceRowsByTokenId.get(row.token.id)?.flatMap((otherRow) => {
        ...
        const otherSpokeBlockchain = blockchainByCentId.get(otherRow.token_instance.centrifugeId);
  ...
  return c.json({
    paging: {
      ...
      prev: offset - limit >= 0 ? `${baseUrl}?limit=${limit}&offset=${offset - limit}` : null,
      next:
...
app.get("/quote", sValidator("query", quoteParams), async (c) => {
  const { fromChainId, toChainId, fromAmount, fromToken } = c.req.valid("query");
  ...
  const hubChainId = blockchains.find((b) => Number(b.centrifugeId) === hubCentId)?.chainId;
  ...
  const [initiateTransferSharesGasLimit, executeTransferSharesGasLimit] = await Promise.all([
    fromClient.readContract({
      ...
      args: [
        ...
        emptyMessage(MessageType.InitiateTransferShares, poolId(fromTokenInstance.token.id)),
      ],
    }),
    hubClient.readContract({
      ...
      args: [
        toCentId,
        emptyMessage(MessageType.ExecuteTransferShares, poolId(fromTokenInstance.token.id)),
      ],
    }),
  ]);
...
app.get("/stats", async (c) => {
  const [tvl, aggregatedSupply] = await Promise.all([
    TokenService.getNormalisedTvl(context),
    TokenService.getNormalisedAggregatedSupply(context),
  ]);
